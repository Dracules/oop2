...
public class Controller {
    ...
    private void OKSetInfo() {
        ...
        if (!LeanerSuperClass.getName().equals("sample.learner"))
        {
    ...
    private int SetFields (Object ob,Class cl, int FieldIndex) {
        try {
            ...
            for (Field field : ObFields) {
                if (field.getName().equals("scientif"))
                {
    ...
    private int GetFieldsName (Class cl, int LabelIndex)
    {
        ...
        for (Field field : ObFields)
        {
            if (field.getName().equals("scientif"))
            {
                ...
                for (Field ScintField : ScintificFields)
                {
                    labels[LabelIndex].setText(ScintField.getAnnotation(HierarchyAnnotation.class).Label());
    ...
    private void Save(ActionEvent event) {
        if (Choise(true)) {
            Serializ[SerTp.ordinal()].Save(learners);
        }
    ...
    private void Load(ActionEvent event) {
        if (Choise(false)) {
            Serializ[SerTp.ordinal()].Load(learners);
            ShowTab();
        }
        SaveBut.setDisable(false);
        LoadBut.setDisable(false);
    ...
    private int GetFieldsInfo (Object ob, Class cl, int i)
    {
        ...
        for (Field field : ObFields)
        {
            if (field.getName().equals("scientif"))
            {
                ...
                for (Field ScintField : ScintificFields)
                {
                    labels[i].setText(ScintField.getAnnotation(HierarchyAnnotation.class).Label());
                    labels[i].setVisible(true);
                    fields[i].setText(ScintField.get(Scientif).toString());
    ...
    private void SelectedInfo(MouseEvent event) {
        ...
        if (index >=0) {
            ...
            int LabelIndex=0;
            Class LeanerClass =learner.getClass();
            Class LeanerSuperClass = LeanerClass.getSuperclass();
            if (!LeanerSuperClass.getName().equals("sample.learner"))
            {
                LabelIndex = ShowLeanerInfo(learner,LeanerSuperClass.getSuperclass() ,LabelIndex);
            }
            LabelIndex=ShowLeanerInfo(learner,LeanerSuperClass,LabelIndex);
            ShowLeanerInfo(learner,LeanerClass,LabelIndex);
        }
    }
            ...
            try {
                ...
                for (Field field : ObFields)
                {
                    if (field.getName().equals("scientif"))
                    {
                        ...
                        for (Field ScintField : ScintificFields)
                        {
                            NameInfolabels[LabelIndex].setText(ScintField.getAnnotation(HierarchyAnnotation.class).Label());
                            NameInfolabels[LabelIndex].setVisible(true);
                            Infolabels[LabelIndex].setText(ScintField.get(Scientif).toString());
                        ...
                        NameInfolabels[LabelIndex].setVisible(true);
                        Infolabels[LabelIndex].setText(field.get(ob).toString());
                        Infolabels[LabelIndex].setVisible(true);
                        LabelIndex++;
                    }
                }
